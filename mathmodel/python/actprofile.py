...
def determineLabels(frame, columns, name, path, cutoff = 50, eps=0.3, samples=10,perplex=10,savefinalclf=False):
	...
	if(savefinalclf):
		joblib.dump(relabelclf, path+"/clfsave-"+name+".pkl");
...
casetype,firstimport = determineLabels(superframe, goodcols, "casetype", imgpath, cutoff = 65, eps=0.3,samples=10,perplex=5,savefinalclf=True);
...
casecol = pd.merge(casecol, superframe[['TUDIARYDAY','TUCASEID']],on='TUCASEID')
