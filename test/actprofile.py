...
def tsneLabelFit(prox, eps = 0.5, samples = 1,perplex=10):
	ncomp = 2;
	tsne = sklearn.manifold.TSNE(n_components = ncomp,perplexity=perplex,early_exaggeration=4.0,verbose=2,metric='precomputed')
...
def determineLabels(frame, columns, name, path, cutoff = 50, eps=0.3, samples=10,perplex=10):
	...
	refitclf,refitlabels = treeRefit(stripvector, striplabels, depth=7, nest=2000);
...
secondcol,secondimport = determineLabels(superframe, fullcols, "second", imgpath, cutoff = 35,eps=0.2,samples=20,perplex=30);

subframe = columnStrip(superframe, goodcols);

finalclf,finallabels=treeRefit(subframe.values,secondcol['secondlabelreduce'].values,depth=None, nest=2000);

secondcol['finallabels']=finallabels;

casecol = secondcol;

#superframe['daytype']=secondcol['secondlabelreduce'];

#firstcol,firstimport = determineLabels(superframe, goodcols, "first", imgpath, cutoff = 65, eps=0.3,samples=10,perplex=5);
...
demoActPlot(labelledacttable, "finallabels", imgpath + "/goodcol-")
