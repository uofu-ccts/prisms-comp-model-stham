...
def slidewindow(table, maxinstances, interval_size,actmapping,locmapping):
	...
	intervals = int(1440 / interval_size);
	...
	#print(avginstances);
	#plt.show();
	#print(actcount,maxinstances,int(intervals))
	...
	for i,df in table.iterrows():
		...
		index = int(np.floor(df.start / interval_size))
		endindex = int(np.floor(df.end / interval_size)-1)
		#print(df.actind,df.instance,index)
...
outfile = h5py.File(datapath + "actdata1.h5");
